...
//define(0x00000021, 0x00000021);
//define(0x00000006, 0x00000006);
//define(0x00000015, 0x00000015);
...
//        const 0x00000004= 0x00000004;
//        const 0x00000021= 0x00000021;
//        const 0x00000006= 0x00000006;
//        const 0x00000015= 0x00000015;
...
*/
	...
	*/
	...
	{
		$pdu = "";
		$status = $this->SendPDU(0x00000015, $pdu);
		return ($status == 0);
	}
...
// private members (not documented):
	...
	function ExpectPDU($our_sequence_number)
	{
		do {
			...
			switch ($command_id) {
                        case 0x00000002:
				$this->debug("Got 0x00000002_RESP.\n");
				$spec = "asystem_id";
				extract($this->unpack2($spec, $pdu));
				$this->debug("system id       : $spec.\n");
				break;
			case 0x00000006:
				$this->debug("Got 0x00000006_RESP.\n");
				break;
			case 0x00000004:
				$this->debug("Got 0x00000004_RESP.\n");
				if ($command_status == 0) {
					$spec = "amessage_id";
					extract($this->unpack2($spec, $pdu));
					$this->debug("message id      : $spec.\n");
				}
				break;
			case 0x00000021:
				$this->debug("Got 0x00000021_RESP.\n");
				$spec = "amessage_id/cno_unsuccess/";
				extract($this->unpack2($spec, $pdu));
				$this->debug("message id      : $spec.\n");
				$this->debug("no_unsuccess    : $spec.\n");
				break;
			case 0x00000015:
				$this->debug("GOT 0x00000015_RESP.\n");
				break;
			default:
	...
	function SendBindTransmitter($system_id, $smpppassword, $system_type)
	{
		...
		$this->debug("\n");
		$status = $this->SendPDU(0x00000002, $pdu);
		return $status;
	}
	...
	{
		$pdu = "";
		$status = $this->SendPDU(0x00000006, $pdu);
		return $status;
	}
	...
	function SendSubmitSM($service_type, $source_addr_ton, $source_addr_npi, $source_addr, $dest_addr_ton, $dest_addr_npi, $destination_addr, $esm_class, $protocol_id, $priority_flag, $schedule_delivery_time, $validity_period, $registered_delivery_flag, $replace_if_present_flag, $data_coding, $sm_default_msg_id, $sm_length, $short_message)
	{
		...
		$pdu = pack($spec,
			...
			$sm_length,
			$short_message);
		$status = $this->SendPDU(0x00000004, $pdu);
		return $status;
	}
